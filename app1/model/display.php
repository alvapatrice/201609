...
class display extends awebarts {
    ...
    public function pagination2($col1,$value1,$col2,$postedby,$start,$extra="",$perPage=9,$order='id desc',$select='*',$where="where")
    { if($where=='where')
        $where.=" $col1 = '$value1' AND $col2='$postedby' ";
        $start=$start-1;
        $start=$start*$perPage;
        if($col1=="cat_id")
            $sql = "select $select from $this->tablename $where  $extra order by $order limit $start,$perPage ";
    ...
    public function get_comments($id, $username) {
        $sql = "SELECT * FROM $this->tablename where `item_id` =:id AND `username` =:username ";
        $query = $this->use->connect->prepare($sql);
    ...
    public function get_actv_users() {
        $sql = "select * from $this->tablename where is_online=1 ";
        $query = $this->use->connect->prepare($sql);
        $query->execute();
        $this->row_section = $query->fetchAll();
        return $this->row_section;
